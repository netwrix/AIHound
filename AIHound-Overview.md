...
AIHound scans a workstation for **25 different categories of AI credentials and secrets** — across files, environment variables, OS keychains, browser storage, shell history, network listeners, and Docker registries — and produces a risk-rated report with **actionable remediation guidance** on every finding. It's read-only, redacts secrets by default, and runs on Windows, macOS, Linux, and WSL.

---

## Coverage at a Glance

### 25 scanners, organized by category:
...
| Level | Means |
|-------|-------|
| **CRITICAL** | Plaintext + world-readable file, OR an unauthenticated AI server listening on `0.0.0.0`, OR an empty Jupyter token |
| **HIGH** | Plaintext credential file with user-only permissions, OR a known API key prefix found in shell history |
...
**v3.0.0** — Production-ready. All features at parity across Python, Go, and PyInstaller. 25 scanners. 85 passing tests. Cross-platform (Windows / macOS / Linux / WSL).
